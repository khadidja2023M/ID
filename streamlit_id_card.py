...
def structure_text(text):
    ...
    # Extracting Name
    name_pattern = r"National Identity Card\s+([A-Za-z]+\s[A-Za-z]+)"
    ...
    # Extracting Issue Place and Issue Date
    issue_data = re.search(r"(London)\s(\d{2}-\d{2}-\d{4})", text)
    if issue_data:
        structured_data["Issue Place"] = issue_data.group(1)
        structured_data["Issue Date"] = issue_data.group(2)

    # Extracting Expiry Date
    expiry_date_pattern = r"(\d{2}-\d{2}-\d{4})\s*$"
    expiry_date = re.search(expiry_date_pattern, text)
    if expiry_date:
        structured_data["Expiry Date"] = expiry_date.group(1)
