...
def structure_text(text):
    ...
    issue_data = re.search(r"([A-Z][a-zA-Z\s]{1,50})\s(\d{2}-\d{2}-\d{4})\s+(\d{2}-\d{2}-\d{4})", text)
    if issue_data:
        structured_data["Issue Place"] = issue_data.group(1).strip()
        structured_data["Issue Date"] = issue_data.group(2)
        structured_data["Expiry Date"] = issue_data.group(3)
