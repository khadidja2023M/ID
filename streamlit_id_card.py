...
def structure_text(text):
    ...
    dob = re.search(r"British Citizen (\d{2}-\d{2}-\d{4})", text)
    if dob:
        structured_data["Date of Birth"] = dob.group(1)

    # Extracting Issue Place and Issue Date
    issue_data = re.search(r"(\b[A-Za-z]+\b) (\d{2}-\d{2}-\d{4})", text)
    if issue_data:
    ...
    if expiry_date and len(expiry_date) > 1:
        structured_data["Expiry Date"] = expiry_date[-1] # Getting the last date
